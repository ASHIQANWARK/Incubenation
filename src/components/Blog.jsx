...
import { PenTool, X, ArrowRight, FileText } from "lucide-react";
...
const blogs = [
  {
    id: 1,
    title: "What is a Startup Incubator and How Does It Work?",
    date: "12-09-2025",
    author: "Shafi Shoukath",
    image: startupImg,
    excerpt:
      "Learn how startup incubators support founders with mentorship, funding access, and ecosystem support.",
    content: `
Introduction
...
A startup incubator is an organization or program designed to help startups in their early stages. Unlike accelerators, which focus on rapid growth, incubators focus on nurturing and supporting startups until they are ready for scaling.

The role of a startup incubator includes:

- Providing affordable office space for startups
- Offering mentorship and training programs for founders
- Connecting startups with investors and industry experts
- Helping with legal, financial, and business setup
- Reducing risk by providing a strong foundation

In short, a startup incubator gives entrepreneurs the right ecosystem to turn ideas into viable businesses.

2. Why Do Startups Need an Incubator?

The journey of a startup is filled with challenges funding shortages, lack of experience, and uncertainty about product-market fit. A startup incubator helps bridge these gaps.

Key reasons why a startup incubator is essential:

- Cost Savings -- Shared infrastructure reduces expenses for startups.
- Expert Guidance -- Mentors help startups avoid common mistakes.
- Networking -- Incubators connect startups with investors, clients, and peers.
- Credibility -- Being part of a reputed startup incubator builds trust.
- Market Fit -- Incubators help refine ideas so startups can fit market demand.

Simply put, without an incubator, many startups would struggle to survive.
...
At IncubeNation, we believe every startup deserves the right environment to grow. Our incubator program is designed to help you at every step of your entrepreneurial journey.

Apply today and let IncubeNation be the incubator where your startup dream takes flight.

FAQs

1. What is the role of a startup incubator?
A startup incubator nurtures early-stage startups by providing mentorship, funding access, and resources.

2. Do incubators invest in startups?
Some incubators offer seed funding, while others connect startups with investors.

3. How long do startups stay in an incubator?
...
`,
  },
  {
    ...
    image: successImg,
    excerpt:
      "Discover how incubators provide structured support, mentorship, and long-term growth opportunities for founders.",
    content: `
    ...
    <section className="relative overflow-hidden py-14 sm:py-16 lg:py-20 ">
        ...
        >
          <div className="mb-4 flex justify-center">
            <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-white border border-slate-200 shadow-sm">
              <PenTool className="text-cyan-600 w-5 h-5" />
            </div>
          </div>
          ...
          <p className="mt-3 text-sm sm:text-base text-slate-600 max-w-2xl mx-auto leading-7">
            Explore ideas, startup insights, founder guidance, and ecosystem learnings from the IncubeNation journey.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 lg:gap-6">
          {sortedBlogs.map((blog, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden hover:-translate-y-1.5 hover:shadow-xl transition-all duration-500"
            >
              <div className="overflow-hidden">
                <img
                  src={blog.image}
                  alt={blog.title}
                  className="w-full h-52 sm:h-56 object-cover transition duration-700 group-hover:scale-105"
                />
              </div>
              ...
              <div className="p-5 sm:p-6">
                ...
                <h3 className="text-lg sm:text-xl font-semibold text-slate-900 mb-3 leading-snug line-clamp-2">
                  {blog.title}
                </h3>

                <p className="text-sm sm:text-base text-slate-600 leading-7 mb-5 line-clamp-3">
                  {blog.excerpt}
                </p>

                <button
                  onClick={() => handleReadMore(blog)}
                  className="inline-flex items-center gap-2 text-sm font-semibold text-cyan-700 hover:text-cyan-900 transition"
                >
                  Read Article
      ...
      <AnimatePresence>
        {showDisclaimer && (
          ...
          >
            ...
            >
              <div className="mb-4 flex items-center justify-center w-12 h-12 rounded-xl bg-slate-100 border border-slate-200">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className="px-4 py-3 border border-slate-200 rounded-xl text-slate-700 hover:bg-slate-50 text-sm font-medium transition flex-1"
                >
                  Cancel
                </button>
                <button
                  onClick={acceptDisclaimer}
                  className="px-4 py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 text-sm font-medium transition flex-1"
                >
                  I Understand
                </button>
              </div>
      ...
      <AnimatePresence>
        {selectedBlog && !showDisclaimer && (
          <motion.div
            className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm overflow-y-auto z-50 p-3 sm:p-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            <motion.div
              ...
              className="bg-white rounded-3xl max-w-4xl mx-auto my-4 overflow-hidden shadow-2xl border border-slate-200"
            >
              {/* Sticky Header */}
              <div className="sticky top-0 bg-white/95 backdrop-blur-md flex justify-between items-center px-5 sm:px-6 py-4 border-b border-slate-200 z-10">
                <h2 className="text-lg sm:text-2xl font-semibold text-slate-900 pr-4 leading-snug">
                  {selectedBlog.title}
                </h2>
                <button
                  onClick={closeBlogDetail}
                  className="flex-shrink-0 text-slate-500 hover:text-slate-800 p-2 rounded-full hover:bg-slate-100 transition"
              ...
              <div className="p-5 sm:p-6 lg:p-8">
                <div className="flex items-center text-slate-500 text-sm mb-5 flex-wrap gap-2">
                  <span>{selectedBlog.date}</span>
                  <span>•</span>
                  <span>By {selectedBlog.author}</span>
                </div>

                <div className="prose prose-sm sm:prose-base max-w-none prose-headings:text-slate-900 prose-p:text-slate-700 prose-p:leading-8 prose-strong:text-slate-900 prose-li:text-slate-700">
                  {selectedBlog.content.split("\n").map((paragraph, index) =>
                    paragraph.trim() ? (
                      <p key={index} className="mb-3 text-slate-700 leading-8 text-sm sm:text-base">
                        {paragraph}
                      </p>
                    ) : (
                ...
                <div className="flex justify-center mt-8">
                  <button
                    onClick={closeBlogDetail}
                    className="px-6 py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 text-sm font-medium transition"
                  >
                    Back to Blog
                  </button>
                </div>
